...
func (s *AuthEmail) CreateResetToken(email string) (string, error) {
	userID, lastSent, err := s.repo.GetTokenResetPassword(email)
	if err != nil {
		log.Printf("Ошибка получения userID для email %s: %s", email, err)
		return "", err
	}

	// Проверяем, можно ли отправить новый токен
	if time.Since(lastSent) < tokenEmail {
		return "", fmt.Errorf("новый токен можно запросить только через %v", tokenEmail)
	}
